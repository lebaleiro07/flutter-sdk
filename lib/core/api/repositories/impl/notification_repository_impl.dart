...
class NotificationRepositoryImpl implements NotificationRepository {
  ...
  Future<List<CentralNotification>> get() async {
    ...
    return data.map<CentralNotification>((notification) => CentralNotification.fromJson(notification)).toList();
    } catch(e, s){
      print("$e $s");
    }
  }
  ...
  @override
  Future<DataWithCursor<CentralNotification>> getById(String notificationId, { int page = 8, String cursor }) async {
    final response = await httpClient.get(
      "${NotificationEndpoint.getById(notificationId)}?limit=$page&" + (cursor != null ? 'next=$cursor' : '')
    );

    final data = jsonDecode(response.body)["data"];
    final dataCursor = Cursor.fromMap(jsonDecode(response.body)["cursor"]);

    final List<CentralNotification> notifications =  data.map<CentralNotification>((notification) => CentralNotification.fromJson(notification)).toList();
  ...
  @override
  Future<CentralNotification> update(String notificationId, CentralNotification notification) async {
    final response = await httpClient.put(NotificationEndpoint.update(notificationId), body: notification.toJSON());
